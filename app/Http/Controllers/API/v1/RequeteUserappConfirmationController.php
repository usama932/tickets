    ...
    public function userCurrentRide(Request $request)
    {
        ...
        $sql = DB::table('tj_requete')
            ...
            ->select('tj_requete.id', 'tj_requete.id_user_app',
                ...
                'tj_requete.destination_name', 'tj_requete.latitude_depart',
        ...
        foreach ($sql_vehicle as $row_vehicle) {
            ...
            $row->brand = Brand::where('id', $row_vehicle->brand)->first();
            $row->model = DriverModel::where('id_conducteur', $sql->id_conducteur)->first();
